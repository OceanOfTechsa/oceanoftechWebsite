// components/ServiceTestimonialCarousel.tsx
"use client";
...
export default function ServiceTestimonialCarousel({
  ...
  // Auto-scroll
  useEffect(() => {
    if (paused || filteredReviews.length === 0) return;
    const interval = setInterval(() => {
      setPage((prev) => (prev + 1) % totalPages);
    }, 4000);
    return () => clearInterval(interval);
  }, [paused, totalPages, filteredReviews.length]);
  ...
  // If no reviews for this service
  if (filteredReviews.length === 0) {
    return (
      <div className="w-full max-w-5xl mx-auto p-6 text-center bg-white dark:bg-[#202124] rounded-2xl shadow-md">
        ...
        <Link
          ...
          target="_blank"
          className="bg-[#09b850] text-white px-6 py-3 rounded-sm font-medium hover:bg-[#08a648] transition inline-flex items-center"
        >
          Leave a Review
        </Link>
      </div>
    ...
    >
      ...
      >
        {currentItems.map((review, i) => (
          <div
            key={i}
            className="flex-shrink-0 basis-[85%] md:basis-[48%] lg:basis-[45%] p-3 rounded-2xl bg-white dark:bg-[#202124] shadow-md"
          >
            {/* Rating */}
        ...
        {currentItems.length === 1 && (
          <div className="hidden md:flex-shrink-0 basis-[85%] md:basis-[48%] lg:basis-[45%] p-3 rounded-2xl bg-white dark:bg-[#202124] shadow-md md:flex flex-col items-center justify-center text-center">
            ...
            <Link
              href="https://g.page/r/CU3CZKX3tetcEAE/review"
              target="_blank"
              className="bg-[#09b850] text-white px-4 py-2 rounded-sm font-medium hover:bg-[#08a648] transition"
            >
              Leave a Review
            </Link>
          </div>
