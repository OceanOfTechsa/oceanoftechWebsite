    ...
    return (
        <ul className="flex space-x-3 -mt-2">
            {socials.map((social, i) => (
                <li key={i}>
                    <Link href={social.url} target="_blank" className={`p-2 ${changeBg ? "bg-gray-200 dark:bg-[#292a2d]" : "bg-[#292a2d]"} rounded-sm active:bg-[#c6c6c6] focus:bg-white focus:text-black hover:bg-[#d3d3d3]/20 transition-colors flex items-center justify-center`}>
                        {social.icon}
                    </Link>
                </li>
            ))}
