...
export function TechStackBeams(): JSX.Element {
  ...
  const centerRef: RefObject<HTMLDivElement | null> =
    useRef<HTMLDivElement>(null);
  const refs: RefObject<HTMLDivElement | null>[] = Array.from(
    { length: 8 },
    (): RefObject<HTMLDivElement | null> => useRef<HTMLDivElement>(null),
  );
    ...
    >
      ...
      <div className="relative z-10 flex h-full w-full max-w-4xl items-center justify-between">
        ...
        <div className="grid grid-cols-2 gap-3 md:flex md:flex-col md:gap-8">
          <div
            ref={refs[0]}
            className="relative flex h-10 w-10 md:h-12 md:w-12 items-center justify-center rounded-full bg-[#161618] shadow-sm"
          >
      ...
      <div className="absolute bottom-4 md:bottom-6 left-0 right-0 text-center top-2">
        <p className="text-xs md:text-sm font-medium text-white">
          Full-Stack <span className="text-green-500">•</span> Modern{" "}
