...
const invoices = [
  {
    invoice: "Supabase incident on February 12, 2026",
    paymentStatus: "Paid",
    totalAmount: "$250.00",
  ...
  {
    invoice: "When to use Read Replicas vs. bigger compute",
    paymentStatus: "Paid",
    totalAmount: "$450.00",
    paymentMethod: "Credit Card",
  },
  ...
  {
    invoice: "Building ChatGPT Apps with Supabase Edge Functions and mcp-use",
    paymentStatus: "Pending",
    totalAmount: "$200.00",
    paymentMethod: "Bank Transfer",
  },
  {
    invoice: 'Build "Sign in with Your App" using Supabase Auth',
    paymentStatus: "Unpaid",
    totalAmount: "$300.00",
    paymentMethod: "Credit Card",
  },
];
const Categories = ["All", "Product", "Company", "Developers"];
      ...
      <div className="flex items-center justify-between w-full px-4 sm:px-0">
        ...
        >
          ...
          <div className="hidden sm:flex gap-2">
            {Categories.map((cat: string, index: number) => (
              <Button
                variant="outline"
                className="rounded-sm h-[30px] bg-transparent"
                key={index}
                onClick={(): void => setSelectedCat(cat)}
              >
                {cat}
              </Button>
            ))}
        ...
        <div className="flex-1 flex items-center justify-end gap-2 w-full">
            ...
            {searchExpanded ? (
              <div className="transition-all duration-300 ease-in-out flex items-center gap-2 w-full">
                <div className="relative w-full">
                  <div className="text-muted-foreground pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2">
          ...
          <div className="hidden sm:flex items-center gap-2 w-full justify-start">
            <div
              className={cn("transition-all duration-300 ease-in-out w-full")}
            >
              <div className="relative w-full">
                <div className="text-muted-foreground pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2">
                  <Search className="size-4" />
            ...
            <Button
              ...
              className="rounded-sm"
              onClick={() =>
                setView((prev: viewType): "grid" | "table" =>
            ...
            <Table>
              <TableBody>
                {invoices.map((invoice) => (
                  ...
                  >
                    {/* Invoice ID */}
                    <TableCell className="w-[200px] py-4 group-hover:underline">
                    ...
                    <TableCell className="w-[100px] py-4">
                      <Badge
                        variant="outline"
                        className="text-xs rounded-full text-[#606261] dark:text-[#c4c5c7]"
                      >
                        Company
                      </Badge>
                    </TableCell>
                    ...
                    {/* Date */}
                    <TableCell className="text-right py-4 text-[#606261] dark:text-[#c4c5c7] group-hover:text-black">
                      13 Feb 2026
                    </TableCell>
                  </TableRow>
                ))}
