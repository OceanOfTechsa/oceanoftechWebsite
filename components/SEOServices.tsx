...
export default function SEOServicesSection() {
  ...
  useEffect(() => {
    if (itemsPerPage >= 3 || paused) return;
    const interval = setInterval(() => {
      setPage((p) => (p + 1) % totalPages);
    }, 5000);
    return () => clearInterval(interval);
  }, [itemsPerPage, paused, totalPages]);

  const getDisplayItems = () => {
    if (itemsPerPage >= 3) {
      return services.map((service, idx) => ({ ...service, originalIndex: idx }));
    }
    const start = page * itemsPerPage;
    const items = [];
    for (let i = 0; i < itemsPerPage; i++) {
      const serviceIndex = (start + i) % services.length;
      items.push({ ...services[serviceIndex], originalIndex: serviceIndex });
    }
    return items;
  };
          ...
          <motion.div
            ...
            className={`grid gap-6 ${
              itemsPerPage === 1
                ...
                : "grid-cols-1 lg:grid-cols-3 lg:items-start"
            }`}
          >
            {currentItems.map((service, index) => {
              const isLastCard = service.originalIndex === 2;
              return (
              <div
                key={`${page}-${index}`}
                className={`${service.bgClass} rounded-[0.2rem] p-4 lg:p-6 transition-transform duration-300 relative last:border ${
                  itemsPerPage === 3 && index === 0 ? 'lg:mt-20' : 
                  itemsPerPage === 3 && index === 1 ? 'lg:mt-10' : ''
                }`}
              >
                {/* Header */}
                <div className="flex justify-between items-start mb-10 text-start">
                  <h3 className={`text-2xl md:text-3xl font-bold pr-4 leading-tight ${
                    isLastCard ? 'text-black dark:text-white' : 'text-white'
                  }`}>
                    {service.title}
                  </h3>
                  <Link
                ...
                <div className="text-start">
                  <span className={`block mb-3 font-medium opacity-80 ${
                    isLastCard ? 'text-black dark:text-white' : 'text-white'
                  }`}>
                    What included:
                  </span>
