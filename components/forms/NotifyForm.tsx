...
const formSchema = z.object({
  ...
  module: z.string("A valid module is required."),
});
...
const NotifyMeForm = ({
  ...
  module,
}: EMailOnlyContactFormProps) => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [emailError, setEmailError] = useState<boolean>(false);

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: { email: "", module: module },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>): Promise<void> {
    ...
    if (results.success) {
      ...
      toast.error(
        results.error?.toString() || "Something went wrong, please try again.",
        { id: toastId },
      );
    }
    setIsLoading(false);
  ...
  return (
    <Form {...form}>
      <form
        ...
        className={`${bg} ${showBorder ? "border" : ""} ${emailError && "border-rose-500"} $ rounded-sm flex items-center p-2 w-full gap-2`}
      >
        <FormField
        ...
        <Button
          type="submit"
          className={`text-white px-4 py-2 rounded-[0.2rem] transition cursor-pointer ${buttonBg}`}
          disabled={isLoading}
        >
          {!isLoading ? (
            <>
