...
const PortfolioCategoryPage = (): React.JSX.Element => {
  const caseStudiesRef: React.RefObject<HTMLDivElement | null> = useRef<HTMLDivElement>(null);
  const { category } = useParams();
  const filteredCaseStudies = useMemo(() => {
    if (!category) return caseStudies;
    const normalizedCategory = category.toString().replace("-", " ").toLowerCase();
    return caseStudies.filter(study => {return study.categories.some(cat => cat.toLowerCase().includes(normalizedCategory) || normalizedCategory.includes(cat.toLowerCase()));});
  }, [category]);
  ...
  return (
    <section className="w-full max-w-7xl xl:py-20 pb-0 mt-10 sm:mt-0 px-4 md:px-0">
      <section className="flex flex-col md:flex-row w-full justify-between items-start md:gap-56">
        <div className="w-full flex flex-col items-start justify-start">
          <nav className="mb-3" aria-label="breadcrumb">
            <ol className="flex items-center gap-2 pt-0">
              ...
              <li className="text-[#0B9944] dark:text-[#09b850]" aria-current="page">
                {category ? category.toString().replace("-", " ") : "Portfolio"}
              </li>
            </ol>
          </nav>
          ...
          <h1 className="text-4xl md:text-[3rem] font-black leading-tight">
            {category
              ? `Highlights in ${category.toString().replace("-", " ")}`
      ...
      <div ref={caseStudiesRef} className="bg-transparent overflow-hidden mb-6 xl:mb-8 mt-[64px]">
        {/* Pass filtered case studies instead of all case studies */}
        <CaseStudiesList 
      ...
      <div className="bg-[#09b850] text-white p-[2.5rem] rounded-sm text-center mt-30 relative overflow-hidden mb-10">
        ...
        <div className="w-full flex flex-col justify-center items-center">
          <h2 className="text-[56.96px] md:text-[2.75rem] font-bold line-height=[1.25] text-white">
            Let&apos;s Work Together
          </h2>
          <p className="mb-3 font-normal text-lg my-[1rem] mx-[1.6rem] text-white">
            We&apos;ll take the time to understand your vision and goals, and work with you to develop a <br /> customized plan to help you succeed.
          </p>
