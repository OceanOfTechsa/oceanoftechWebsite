...
export async function generateMetadata({
  ...
  const job: Career | undefined = careers?.find(
    (career: Career): boolean => career.slug === params.slug,
  );
  if (!job) {
    return {
      title: "Job not found",
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://${AppSettings.COMPANY_DOMAIN}/careers/${job.slug}`,
      type: "website",
      images: [
        {
    ...
    <div className="flex flex-col w-full">
      <section className="w-full max-w-5xl mx-auto px-4 py-20 text-center xl:py-20 pb-0">
        ...
        <div className="bg-[#202124] rounded-sm p-[2.5rem] w-full mb-[3.5rem]">
          <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
            <div className="flex flex-col gap-1 md:gap-2 items-start justify-center">
              ...
              <div className="flex flex-wrap gap-4 text-[#c4c5c7]">
                ...
                <span className="flex items-center gap-2">
                  ...
                  {job.education.map(
                    (
                      education: Career["education"][number],
                      index: number,
                    ): JSX.Element => (
            ...
            <Link
              ...
              className="inline-flex items-center justify-center w-full sm:w-auto bg-[#0B9944] hover:bg-[#09B850] text-white px-5 py-2 rounded-sm transition"
            >
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {/* Skills */}
          <div>
            <h6 className="font-bold mb-[0.5rem] text-start text-[1.2rem] line-height-[1.25]">
              Skills:
            </h6>
            <ul className="space-y-[1rem]">
              {job.skills.map((skill: Career["skills"][number], i: number) => (
                <li
                  className="flex items-start text-[#606261] font-semibold"
                  key={i}
                >
          ...
          <div>
            ...
            <ul className="space-y-[1rem]">
              {job.responsibilities.map(
                (
                  responsibility: Career["responsibilities"][number],
                  i: number,
                ) => (
                  <li
                    className="flex items-start text-[#606261] font-semibold"
          ...
          <div>
            ...
            <ul className="space-y-[1rem]">
              {job.education.map(
                (edu: Career["education"][number], i: number) => (
                  <li
                    className="flex items-start text-[#606261] font-semibold"
                    key={i}
                  >
          ...
          <div>
            ...
            <ul className="space-y-[1rem]">
              {job.experience.map(
                (exp: Career["experience"][number], i: number) => (
                  <li
                    className="flex items-start text-[#606261] font-semibold"
                    key={i}
                  >
        ...
        <div className="mt-8">
          ...
          <p className="text-start">{job.additionalRequirements}</p>
        </div>
        ...
        <div className="mt-8">
          ...
          <p className="text-start">{job.description}</p>
        </div>
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 items-center gap-8">
            ...
            <div className="xl:col-span-2">
              ...
              <p className="text-[#606261] dark:text-[#c4c5c7] text-lg">
                Our passion for customer excellence is just one reason why we
                are the market leader. We&apos;ve <br />
                always worked very hard to give our customers the best
                experience.
              </p>
            </div>
